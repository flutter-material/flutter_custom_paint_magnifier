...
import '../utilities/magnifier.dart';
...
class _MeasureObjectState extends State<MeasureObject> {
  // 放大鏡所需變數
  late Offset position = const Offset(250, 200);
                    ...
                    onPanUpdate: (DragUpdateDetails details) {
                      if (_currentlyDraggedIndex != -1) {
                        setState(() {
                          ...
                          _points[_currentlyDraggedIndex] = Offset(x1, y1);
                          // 計算兩點距離
                          point_distance = sqrt(
                              pow(_points[0].dx - _points[1].dx, 2) +
                                  pow(_points[0].dy - _points[1].dy, 2));
                          // 啟用放大鏡功能
                          magnifierVisible = true;
                          position = Offset(details.globalPosition.dx,
