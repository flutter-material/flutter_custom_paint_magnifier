...
class MyMagnifier extends StatefulWidget {
  ...
  @override
  _MyMagnifierState createState() => _MyMagnifierState();
}
...
class _MyMagnifierState extends State<MyMagnifier> {
  ...
  @override
  void didUpdateWidget(MyMagnifier oldWidget) {
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [
        widget.child,
        if (widget.visible && widget.position != null) _getMyMagnifier(context)
      ],
    );
  }
  ...
  void _calculateMatrix() {
    ...
    setState(() {
      ...
      if (_bubbleCrossesMyMagnifier()) {
        // final box = context.findRenderObject() as RenderBox;
        Size s = (context.findRenderObject() as RenderBox?)?.size ?? Size.zero;

        newX -= ((s.width - _magnifierSize.width) / _scale);
        print("ssadsa");
      }
  ...
  Widget _getMyMagnifier(BuildContext context) {
    return Align(
  ...
  bool _bubbleCrossesMyMagnifier() =>
